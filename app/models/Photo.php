...
{
    public function all(){
        $sql = "SELECT photos.id as id, photos.path as path, photos.user_id as user_id, photos.likes as likes, photos.created_at as created_at, users.login as user FROM photos INNER JOIN users ON photos.user_id = users.id;";
